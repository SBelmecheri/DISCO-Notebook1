...
4. A file containing regional Vapor pressure deficit data. The filename is "CRU_vpd.csv"
...
An example with March through June temperatures is shown below using the ```"rowMeans()"``` function. Unfortunately, this function removes the time-series information so we need to add this back with the ```"ts()"``` function.
