...
Correction and Interpretation of stable Carbon isotopes
COURSE: DISC, LTRR, UNIVERSITY OF ARIZONA. INSTRUCTORS: S. BELMECHERI & P. SZEJNER

The following code and comments should further your experience to analyze/treat tree ring d13C data (including atmosphere and climate data). This exercise relies upon raw tree-ring d13C measurements, atmospheric d13C measurements and instrumental climate data.

There are a few questions throughout to catalyze understanding and critical thinking.
The analysis is broken into 4 main parts (including 17 questions) - at the end you should have
gone through many of the steps commonly used for treating, correcting and interpreting a d13C time series.

You will need to download the R project from https://github.com/SBelmecheri/DISCO-Notebook1.git and to set your own working directory. Once you open the Notebook1.Rproj in R studio  you can open the Notebook1.Rmd file to proceed with this exercise.
...
# The command "setwd" is used for this as in the following line. You will need to change it to your working directory.
setwd("~/Documents/DISC/NOTEBOOK1")
...
# TIME: 1901-2002. ELEVATION: 1600m a.s.l. LATITUDE: 43.05. LONGITUDE: -5.25. SPECIES: Pinus sylvestris.
# STAND STRUCTURE: Open forest. ASPECT: NW. SLOPE: 28 degrees.

# You will also need to ensure that the name of the file is correct.
TR13 <- read.table("LIL.txt", header=T)

#convert to a time-series object. 
TR13.ts <- ts(TR13,start=1900, frequency=1)
...
ts.plot(TR13.ts,ylab="d13C (permil.VPDB)",xlab="Years",xlim=c(1900,1960), col="#9C964A",lty=1, lwd=2)
ts.plot(TR13.ts,ylab="d13C (permil.VPDB)",xlab="Years",xlim=c(1960,2002), col="#9C964A",lty=3,lwd=2)
```
...
#convert to a time-series object
ATM13.ts <- ts(ATM13,start=1900,frequency=1)

#plot  the atmospheric d13C dataset
ts.plot(ATM13.ts,ylab="d13C (permil.VPDB)",xlab="Years")
```
...
layout(matrix(1:2,nrow = 1, ncol = 2,byrow = T), widths = c(3,3),heights = c(3,3),respect = TRUE)
ts.plot(TR13.ts,ylab="d13C (permil.VPDB)",xlab="Years", col="#9C964A",main="TREE RINGS")
ts.plot(ATM13.ts,ylab="d13C (permil.VPDB)",xlab="Years", main="ATMOSPHERE")
```
...
This change corresponds to a decline in atmospheric d13C related to burning of fossil fuels (enriched in 12C).
This trend or effect is termed Suess Effect (Keeling, 1979), is recorded in tree-ring raw d13C data, and should therefore be removed before analyzing climate or ecophysiology of a tree ring d13C chronology.
...
# to a pre-industrial standard value of 6.5 permil i.e. remove the trend of 13C that result from a decline due to the Suess effect.
...
# Note that you may need to adjust the limits of the Y axis to accommodate the values-range of raw and corrected 13C data
# you can do that using ylim=c(,) to set min and max Y axis values.
```

Question 7: How does the corrected d13C compare to the raw d13C. 


The correction of raw 13C tree ring from the Suess effect yields a d13C chronology that contains 2 signals : atmospheric CO2 and climate.
This is related to tree physiological response to these environmental variables as was reviewed in the stable isotope theory lectures.
Tree ring d13C chronologies can be corrected for changes in the carbon dioxide content of the atmosphere.
This allows analysis and interpretation of d13C variations and trends in response to climate variability/trends only.

This correction, termed PIN correction (for preindustrial) corresponds to a statistical procedure that calculate the d13C values that would have been obtained under pre-industrial conditions (pre-increase in atmospheric CO2)
This procedure has been published by McCarroll et al. 2008 [https://doi.org/10.1016/j.gca.2008.11.041]
...
# Note you can rerun this function and set the plot.it argument as TRUE. This will allow you top see plots generated for the various steps of the PIN calculation. You will need to hit return mulyiple times.
# TR13.corr.pin <- pin(c(1901:2002), TR13.corr, co2, alpha = 0.9, firstyear =1901, lastyear =2002 , plot.it = TRUE, verbose = TRUE, tol = 0)
```
...
Explore the climate data (seasonality, trends). Below is an example for Temperature:
```{r}
...
Question 10: Do you observe a trend in the temperature record?
...
Tree growth and physiology respond to climate variation often over a variety of months. When we are investigating the climate signal recorded in d13C or reconstructing climate, 
we often try to understand the climate response better by checking correlations over a wide variety of seasons. This is most easily done by averaging months together and computing correlations. 

An example with March through June temperatures is shown below using the "rowMeans" command. This command removes the time-series information so we need to add this back with the "ts" command.
Below this is shown in two steps overwriting the initial variable (tt.Mar_Jun)  instead of defining a new one.
```{r}
tt.Mar_Jun <- rowMeans(gridded.tt.data[,c(3,4,5,6)])
tt.Mar_Jun <- ts(tt.Mar_Jun,start=1901,frequency =1)
```

Other code for this would be:

```{r}
tt.Mar_Jun <- rowMeans(gridded.tt.data[,c("Mar","Apr","May","Jun")])
tt.Mar_Jun <- rowMeans(gridded.tt.data[,3:6])
```
...
We have seen above that d13C time series can be treated to remove the effect of declining atmospheric d13C and the effect of increasing atmospheric CO2 concentration (PIN). The Suess Correction is straightforward and necessary as it avoids artifactual trends in tree ring d13C. 
...
Another way to treat the d13C data is to calculate isotopic discrimination. This takes care of the Suess effect and allows interpreting variation of the isotopic fractionation in term of photosynthesis processes and  availability of leaf internal CO2
...
Question 17: compare the d13C (tree rings d13C corrected from Suess effect) and D13C (isotopic discrimination). Describe the trends in terms of isotopic fractionation and associated physiological responses.

Use the climate correlations investigated above to discuss how tree physiology is responding to changes in trends and variability of the climate factors and atmospheric CO2 concentration.
